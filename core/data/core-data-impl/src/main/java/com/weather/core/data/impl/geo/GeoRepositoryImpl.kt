...
class GeoRepositoryImpl(
    ...
    private val _downloadedCities = MutableStateFlow<GeoDomain?>(null)
    override val downloadedCities = _downloadedCities.filterNotNull()

    private val _downloadedNextCities = MutableStateFlow<GeoDomain?>(null)
    ...
    override suspend fun downloadCities(namePrefix: String) {
        return safeApiCall {
            ...
            _downloadedCities.emit(mapper.toDomain(response))
        }
    }
    ...
    override suspend fun downloadNextCities(geoLink: GeoLinkDomain) {
        safeApiCall {
            ...
            _downloadedNextCities.emit(mapper.toDomain(response))
        }
    }
